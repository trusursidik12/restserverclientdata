...
class Bekasi extends RestController {

    public function aqmdata_post()
    {
        $id_stasiun = $this->post('id_stasiun');
        $waktu = $this->post('waktu');
        $aqmdata = $this->bekasi_m->get($id_stasiun, $waktu);
        if($aqmdata){
            $update = $this->bekasi_m->update_aqmdata($id_stasiun, $waktu);
            if($update){
            ...
            $insert = $this->bekasi_m->add_aqmdata();
            if($insert){
                $aqmdata = $this->bekasi_m->get($id_stasiun, $waktu);
